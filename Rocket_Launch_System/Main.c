...
static double read_double_with_default(const char *prompt, double def, double min, double max) {
    char buf[128];
    double val;
    while (1) {
        ...
        if (buf[0] == '\n') val = def;
        else val = strtod(buf, NULL);
        if (val >= min && (max < min || val <= max)) return val;
        if (max >= min)
            printf("Value out of range: expected between %g and %g. Please try again.\n", min, max);
        else
            printf("Value out of range: expected >= %g. Please try again.\n", min);
    }
}

static void launch_gui(void) {
    char exe_path[4096], exe_dir[4096], gui_script[4096], python_path[4096], command_line[8192];
    STARTUPINFOA si;
    ...
    strcpy(gui_script, exe_dir);
    strcat(gui_script, "\\..\\RocketLaunch_GUI.py");

    if (GetFileAttributesA(gui_script) == INVALID_FILE_ATTRIBUTES) {
        strcpy(gui_script, exe_dir);
        strcat(gui_script, "\\RocketLaunch_GUI.py");
    }

    if (SearchPathA(NULL, "pythonw.exe", NULL, MAX_PATH, python_path, NULL) == 0) {
        if (SearchPathA(NULL, "python.exe", NULL, MAX_PATH, python_path, NULL) == 0) {
            MessageBoxA(NULL, "Python non trovato. Installa Python oppure aggiungilo al PATH.", "Rocket Launch System", MB_ICONERROR | MB_OK);
            return;
        }
    }

    snprintf(command_line, sizeof command_line, "\"%s\" \"%s\"", python_path, gui_script);

    ZeroMemory(&si, sizeof(si));
    ZeroMemory(&pi, sizeof(pi));
    si.cb = sizeof(si);

    if (!CreateProcessA(NULL, command_line, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, exe_dir, &si, &pi)) {
        MessageBoxA(NULL, "Impossibile avviare l'interfaccia grafica.", "Rocket Launch System", MB_ICONERROR | MB_OK);
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    double drag_coeff, ref_area, scale_height = 8500.0, rho0 = 1.225;
    double alt_start = 0.0;

    // Read user-configurable aerodynamic parameters (with defaults and validation)
    drag_coeff = read_double_with_default("Drag coefficient (adim)", 0.5, 0.0, 5.0);
    ref_area   = read_double_with_default("Reference area (m^2)", 10.75, 1e-6, 1e6);
    alt_start  = read_double_with_default("Starting altitude (m)", 0.0, 0.0, 1e7);
    ...
    double x = 0, y = R + alt_start, vx = 0, vy = 0, pitch = 90, time = 0, dt = 0.1;
    int stage = 0, phase = 0;
    double ign_time = 0;
    ...
    char params[12288];
    // pass only the data/output paths; the .py file is the target being opened
    sprintf(params, "\"%s\" \"%s\"", csv_path, images_dir);
    HINSTANCE res = ShellExecuteA(NULL, "open", traj_script, params, NULL, SW_SHOWNORMAL);
    if ((intptr_t)res <= 32) {
        ShellExecuteA(NULL, "open", traj_script, params, NULL, SW_SHOWNORMAL);
    }
    ...
    char params2[12288];
    sprintf(params2, "\"%s\" \"%s\"", csv_path, images_dir);
    HINSTANCE res2 = ShellExecuteA(NULL, "open", grafic_script, params2, NULL, SW_SHOWNORMAL);
    if ((intptr_t)res2 <= 32) {
        ShellExecuteA(NULL, "open", grafic_script, params2, NULL, SW_SHOWNORMAL);
    }
