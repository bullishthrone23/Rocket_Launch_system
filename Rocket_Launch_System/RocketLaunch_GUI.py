...
class RocketLaunchGUI(tk.Tk):
    ...
    def _build_layout(self):
        ...
        ttk.Label(
            header,
            text="Enter simulation parameters, start the C engine and let the graphs be generated automatically.",
    ...
    def _build_general_section(self, parent):
        section = ttk.Labelframe(parent, text="General parameters", style="Section.TLabelframe")
        ...
        ttk.Label(section, text="Mission", style="Section.TLabel").grid(row=2, column=1, sticky="w", pady=(0, 4), padx=(8, 4))
        ...
        ttk.Label(section, text="Number of stages", style="Section.TLabel").grid(row=2, column=2, sticky="w", pady=(0, 4), padx=(8, 4))
        ...
        ttk.Button(exe_row, text="Browse", style="Neutral.TButton", command=self._browse_executable).grid(row=0, column=1, padx=(8, 0))

    def _build_stages_section(self, parent):
        section = ttk.Labelframe(parent, text="Rocket stages", style="Section.TLabelframe")
        ...
        ttk.Label(header, text="Each row corresponds to one stage of the simulation.", style="Card.TLabel").grid(row=0, column=0, sticky="w")
    ...
    def _build_actions_section(self, parent):
        ...
        ttk.Button(actions, text="Update stages", style="Neutral.TButton", command=self._sync_stage_count).grid(row=0, column=0, sticky="w")
        ttk.Button(actions, text="Save profile", style="Neutral.TButton", command=self._save_profile).grid(row=0, column=1)
        ttk.Button(actions, text="Load profile", style="Neutral.TButton", command=self._load_profile).grid(row=0, column=2)
        ttk.Button(actions, text="Start simulation", style="Accent.TButton", command=self._start_simulation).grid(row=0, column=3, sticky="e", padx=(10, 0))
    ...
    def _build_console_section(self, parent):
        section = ttk.Labelframe(parent, text="Execution output", style="Section.TLabelframe")
    ...
    def _sync_stage_count(self):
        try:
            ...
            messagebox.showerror("Error", "The number of stages is not valid.")
            return

        if count < 1:
            messagebox.showerror("Error", "The number of stages must be at least 1.")
            return
    ...
    def _set_stage_count(self, count):
        ...
        for index in range(count):
            stage_frame = ttk.Labelframe(self.stages_container, text=f"Stage {index + 1}", style="Section.TLabelframe")
        ...
        self.status_var.set(f"Configured {count} stages.")
    ...
    def _start_simulation(self):
        if self._run_thread and self._run_thread.is_alive():
            messagebox.showinfo("Simulation in progress", "Wait for the current simulation to finish.")
            return
        ...
        self._append_console(f"Starting {exe_path}\n")
    ...
    def _run_simulation(self, exe_path, csv_path, images_dir, payload):
        try:
            ...
            self.after(0, lambda: self._finish_run(error=f"Error running executable: {exc}"))
            return
    ...
    def _save_profile(self):
        try:
            # collect current inputs into a dict
            stages = []
            for stage_vars in self._stage_vars:
                stages.append({
                    "dry_mass": float(stage_vars[0].get()),
                    "fuel_mass": float(stage_vars[1].get()),
                    "thrust": float(stage_vars[2].get()),
                    "burn_time": float(stage_vars[3].get()),
                    "target_pitch": float(stage_vars[4].get()),
                    "change_rate": float(stage_vars[5].get()),
                })

            profile = {
                "drag_coeff": float(self.drag_coeff.get()),
                "ref_area": float(self.ref_area.get()),
                "alt_start": float(self.alt_start.get()),
            ...
            messagebox.showerror("Save profile", f"Cannot collect inputs: {exc}")
            return
        ...
        try:
            ...
            messagebox.showinfo("Save profile", f"Profile saved: {Path(path).name}")
        except Exception as exc:
            messagebox.showerror("Save profile", f"Failed to save profile: {exc}")
    ...
    def _load_profile(self):
        ...
        try:
            ...
            messagebox.showerror("Load profile", f"Failed to read profile: {exc}")
            return

        try:
            self.drag_coeff.set(str(profile.get("drag_coeff", self.drag_coeff.get())))
            self.ref_area.set(str(profile.get("ref_area", self.ref_area.get())))
            self.alt_start.set(str(profile.get("alt_start", self.alt_start.get())))
            self.mission.set(profile.get("mission", self.mission.get()))
            self.sim_time.set(str(profile.get("sim_time", self.sim_time.get())))
            ...
            for idx, st in enumerate(stages):
                if idx >= len(self._stage_vars):
                    break
                vars_for_stage = self._stage_vars[idx]
                vars_for_stage[0].set(str(st.get("dry_mass", vars_for_stage[0].get())))
                vars_for_stage[1].set(str(st.get("fuel_mass", vars_for_stage[1].get())))
                vars_for_stage[2].set(str(st.get("thrust", vars_for_stage[2].get())))
                vars_for_stage[3].set(str(st.get("burn_time", vars_for_stage[3].get())))
                vars_for_stage[4].set(str(st.get("target_pitch", vars_for_stage[4].get())))
                vars_for_stage[5].set(str(st.get("change_rate", vars_for_stage[5].get())))

            messagebox.showinfo("Load profile", f"Profile loaded: {Path(path).name}")
        except Exception as exc:
            messagebox.showerror("Load profile", f"Failed to apply profile: {exc}")
    ...
    def _finish_run(self, returncode=None, output="", error=None):
        ...
        if returncode == 0:
            self._append_console("Simulation completed. Graphs should open automatically.\n")
            self.status_var.set("Simulation completed.")
            messagebox.showinfo("Completed", "Simulation completed successfully.")
        else:
            self._append_console(f"Process returned exit code {returncode}.\n")
            self.status_var.set("Error during simulation.")
            messagebox.showerror("Error", f"Process returned exit code {returncode}.")
